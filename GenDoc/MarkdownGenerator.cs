...
namespace Heirloom.GenDoc
{
    ...
    public class MarkdownGenerator : DocumentationGenerator
    {
        ...
        protected override string GenerateEnumBody()
        {
            ...
            var names = CurrentType.GetEnumNames();
            markdown += GenerateTable("Name", "Description", names.Select(name =>
            {
                var member = CurrentType.GetMember(name).First();
                var docs = GetSummary(member);
                return (name, docs);
            }));
